...
## QA-003 · 전체 기능 정적 분석 (북마클릿 포함)

**날짜:** 2026-04-17  
**담당:** QA 에이전트  
**실행 케이스:** TC-001 전체, TC-002 전체, TC-003-01/02/04/05, TC-004-01/02/04/05, TC-005 전체, TC-006-01/03

### 통과

TC-001-01~08, TC-002-01~05, TC-003-01/02/04/05, TC-004-01/02/04/05, TC-005-01~04, TC-006-01/03 — **27/27 통과**

### 버그

| ID | TC | 증상 | 심각도 | 상태 |
|----|----|----|--------|------|
| B-006 | TC-006 | 북마클릿 → 프로필 미등록 → 프로필 저장 완료 후 JD 텍스트 사라짐. `saveProfile()` 내 pending JD 복원 로직 없음 | 🟡 Medium | ✅ 수정됨 |
| B-008 | TC-006 | `decodeURIComponent(params.get('jd'))` 이중 디코딩. `URLSearchParams.get()`은 이미 디코딩 수행. `%` 포함 텍스트에서 `URIError` 발생 가능 | 🟡 Medium | ✅ 수정됨 |

### 수정 내용

- **B-006**: `saveProfile()` 마지막에 `jda_pending_jd` 확인 후 `jd-text`에 자동 채움 + 완료 메시지 표시
- **B-008**: `decodeURIComponent(jd)` → `jd` 직접 사용으로 변경

### 브라우저 검증 필요 (정적 분석 한계)

| TC | 이유 |
|----|------|
| TC-001-03 | mammoth.js 실제 DOCX 파싱 동작 확인 필요 |
| TC-003-03 | corsproxy.io 일반 사이트 실제 응답 확인 필요 |
| TC-006-01/02 | 원티드/잡코리아 실제 페이지에서 북마클릿 동작 확인 필요 |
