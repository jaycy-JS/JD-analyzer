...
## ADR-008 · Google Ads 연동 가능성 검토

**날짜:** 2026-04-17  
**상태:** 🔴 현재 아키텍처에서 불가 → 조건부 가능

**컨텍스트**  
JD 분석기에 Google Ads를 연동하는 방향이 검토됨.  
연동 목적에 따라 기술 요구사항이 완전히 달라짐.

**목적별 분석**

| 목적 | 방식 | 현재 구조 가능 여부 |
|------|------|-------------------|
| 앱 내 광고 수익화 | Google AdSense 삽입 | ✅ 가능 (script 태그 추가) |
| 광고 클릭 전환 추적 | Google Ads 전환 태그 | ✅ 가능 (gtag.js 추가) |
| 광고 캠페인 데이터 열람 | Google Ads API | ❌ 불가 (서버 필요) |
| 채용 광고 JD 자동 수집 | Google Ads API + 크롤링 | ❌ 불가 (서버 + 인증 필요) |

**현실적인 구현 방향**

**옵션 A — AdSense 광고 수익화 (가능, 단순)**
- `<script>` 태그만 추가하면 구현 가능
- 단, AdSense 승인에 콘텐츠 정책 충족 필요 (현재 도구 성격상 승인 불확실)
- UX 저하 우려. 소규모 내부 도구에는 과한 선택.

**옵션 B — Google Ads API 연동 (불가, 아키텍처 변경 필요)**
- OAuth 2.0 인증 + API 키 → 서버사이드 필수
- Developer Token 신청 필요 (승인까지 수주 소요)
- 현재 "서버 없음" 원칙(ADR-001)과 충돌 → 백엔드 레이어 구축 필요

**결정**  
현재 단계에서는 Google Ads 연동 **보류**.  
향후 도구가 외부 공개·유료화 단계로 전환될 때 옵션 A(AdSense)부터 검토.

**다음 단계 조건**  
- 월 방문자 1,000+ 이상 → AdSense 신청 검토  
- 백엔드 구축 결정 시 → Google Ads API 연동 재검토 (별도 ADR 작성)
